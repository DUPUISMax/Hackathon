...
import threading
import time
with open('donnee.json', 'r') as file:
    data = json.load(file)
with open('arrets.json', 'r') as file:
    arrets = json.load(file)
personne, temps, lat, lon, arretlat, arretlon, nom, distance, bus = [], [], [], [], [], [], [], [], []
with open('lebus.json', 'r') as file:
    lebus = json.load(file)
for i in range(len(data)):
    entree = data[i]["Nombre_gens"]["Badgeuse"]
    sortie = data[i]["Nombre_gens"]["Capteur_Infra"]
